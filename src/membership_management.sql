...
WITH temp_table AS (
  SELECT m.member_id, ms.type AS membership_type
  FROM members m 
  LEFT JOIN memberships ms ON m.member_id = ms.member_id
)

SELECT t.membership_type,
...
FROM members m 
INNER JOIN memberships ms ON m.member_id = ms.member_id
WHERE ms.end_date BETWEEN datetime('now', 'localtime')
                  AND datetime('now', '365 days') 
